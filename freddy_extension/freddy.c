...
PG_FUNCTION_INFO_V1(hamming_in_batch_fast);

Datum hamming_in_batch_fast(PG_FUNCTION_ARGS) {
  const float MAX_DIST = 1000.0;
...
PG_FUNCTION_INFO_V1(hamming_in_batch_list);

Datum hamming_in_batch_list(PG_FUNCTION_ARGS) {
  const float MAX_DIST = 1000.0;
  ...
  if (SRF_IS_FIRSTCALL()) {
    ...
    //return topKs
    usrfctx = (UsrFctxBatch*)palloc(sizeof(UsrFctxBatch));
    fillUsrFctxBatch(usrfctx, queryIds, queryVectorsSize, resultTopKs, k);
    funcctx->user_fctx = (void*)usrfctx;
    outtertupdesc = CreateTemplateTupleDesc(3, false);
...
Datum cluster_pq(PG_FUNCTION_ARGS) {
  // input: array of ids to cluster, number of clusters
  // output: set of cluster vectors -> arrays of ids corresponding to cluster
  // vectors
  ...
  if (SRF_IS_FIRSTCALL()) {
    ...
    for (int iteration = 0; iteration < iterations; iteration++) {
      ...
      sprintf(command,
              "SELECT pq_quantization.id, pq_quantization.vector, vecs.vector "
              "FROM %s AS pq_quantization INNER JOIN %s AS vecs ON vecs.id = "
              "pq_quantization.id WHERE pq_quantization.id IN (",
              tableNamePqQuantization, tableName);
      // fill command
      cur = command + strlen(command);
      for (int i = 0; i < inputIdsSize; i++) {
        if (i == inputIdsSize - 1) {
